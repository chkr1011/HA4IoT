        ...
        private async Task SendToAzureEventHubAsync(JsonObject data)
        {
            try
            {
                ...
                using (var content = CreateContent(data))
                {
                    ...
                    {
                        NotificationHandler.Verbose("Azure event published successfully.");
                    }
                    else
                    {
                        NotificationHandler.Warning("Failed to publish azure event (Error code: {0}).", result.StatusCode);
                    }
                }
            }
            catch (Exception exception)
            {
                NotificationHandler.Warning("Failed to publish azure event. {0}", exception.Message);
            }
        }
